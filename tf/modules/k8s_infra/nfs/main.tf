...
locals {
  ...
  _nfs_opts = "*(rw,async,no_subtree_check,no_auth_nlm,insecure,no_root_squash,nohide,crossmnt)"
...
resource "kubernetes_config_map" "nfs_exports" {
  ...
  data = {
    "exports" = <<EOF
/nfs *(rw,fsid=0,async,no_subtree_check,no_auth_nlm,insecure,no_root_squash,nohide,crossmnt)
${local._reexport}
EOF
    "binds" = <<EOF
/physical /nfs
